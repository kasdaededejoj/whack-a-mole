...
function sonidoAppear(el) {
  ...
  let f = 0;
  const flickers = [
    () => { el.style.transform = 'translate(-50%,-50%) scale(1.2)'; el.style.filter = 'saturate(0) brightness(4) invert(1)'; },
    () => { el.style.transform = 'translate(-50%,-50%) scale(0.3)'; el.style.opacity = '0'; el.style.filter = 'none'; },
    () => { el.style.transform = 'translate(-50%,-50%) scale(1.1)'; el.style.opacity = '1'; el.style.filter = 'saturate(0) brightness(3)'; },
    () => { el.style.transform = 'translate(-50%,-50%) scale(0.2)'; el.style.opacity = '0'; el.style.filter = 'none'; },
...
function noiseGlitchDisappear(el) {
  ...
  const frames = [
    ...
    () => { el.style.transform = 'translate(-50%,-50%) scale(1.15) skewX(8deg)'; el.style.opacity = '0.7'; el.style.filter = 'saturate(0) brightness(4) invert(1)'; },
    () => { el.style.transform = 'translate(-50%,-50%) scale(1.3) skewX(-6deg)'; el.style.opacity = '0.5'; el.style.filter = 'saturate(2) brightness(5) hue-rotate(270deg)'; },
